...
const apiRoutes = {
  ...
  postRecipientMessage: (team, recipientId) =>
    `/${team}/recipients/${recipientId}/messages/`,
  getRecipientReactions: (team, recipientId) =>
    `/${team}/recipients/${recipientId}/reactions/`,
  postRecipientReaction: (team, recipientId) =>
    `/${team}/recipients/${recipientId}/reactions/`,
};
...
const api = {
  ...
  // Messages
  getMessages: (team, messageId) =>
    apiClient.get(apiRoutes.getMessages(team, messageId)),
  putMessage: (team, messageId, data) =>
    apiClient.put(apiRoutes.putMessage(team, messageId), data),
  patchMessage: (team, messageId, data) =>
    apiClient.patch(apiRoutes.patchMessage(team, messageId), data),
  deleteMessage: (team, messageId) =>
    apiClient.delete(apiRoutes.deleteMessage(team, messageId)),

  // Recipients
  getRecipients: (team) => apiClient.get(apiRoutes.getRecipients(team)),
  postRecipient: (team, data) =>
    apiClient.post(apiRoutes.postRecipient(team), data),
  getRecipient: (team, recipientId) =>
    apiClient.get(apiRoutes.getRecipient(team, recipientId)),

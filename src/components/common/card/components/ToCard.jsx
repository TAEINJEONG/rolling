...
const ToCard = ({ name, profileImages = [], count = 0, reactions = [], bgColor, bgImage }) => {
  ...
  return (
    <section
      className={`${bgImage ? '' : getBgColor(bgColor) || 'bg-beige-200'} relative z-[-2] overflow-hidden w-[275px] h-[260px] rounded-2xl mb-3 border-1 border-black/10 shadow-[0_2px_12px_0_rgba(0,0,0,0.08)]`}
      style={bgImageStyle}
    >
      {!bgImage && (
        <img
      ...
      <article className="px-6 pb-5 pt-7.5 z-10">
        ...
        <span
          className={`inline-block text-16-regular mt-[52px] mb-9 ${bgImage ? 'text-white' : 'text-gray-900'}`}
        >
